...
def main():
    ...
    reader = png.Reader('Test_1bpp_grayscale.png')
...
def start_threads(image):
    ...
    for i in range(num_threads):
        ...
        processes[i] = Process(target=worker, args=(image,start,stop))
...
def create_blur(image_2d, rows_start, rows_stop):
    ...
    #    step1[i] = np.copy(out[i])    #todo do gobal and copy
    #    out[i,:] = 0
    barrier.wait()

    convolve(step1, kernelY, rows_start, rows_stop, step2)
    #for i in range(rows_start, rows_stop):
    #    step2[i] = np.copy(out[i])    #todo do gobal and copy
    #    out[i,:] = 0
    for i in range(rows_start, rows_stop):
...
def create_gradients(rows_start, rows_stop):
    ...
    #for i in range(rows_start, rows_stop):
    #    image_newX[i] = np.copy(out[i])    #todo do gobal and copy
    #    out[i,:] = 0
    barrier.wait()

    convolve(image_blur, sobel_filterY, rows_start, rows_stop, image_newY)
    #for i in range(rows_start, rows_stop):
    #    image_newY[i] = np.copy(out[i])    #todo do gobal and copy
    #    out[i,:] = 0
    for i in range(rows_start, rows_stop):
...
def non_max_suppression(rows_start, rows_stop):
    ...
    global image_out
    for i in range(len(image_out)-1):
        if rows_start > i:
            continue
        if rows_stop <= i:
            return
        for j in range(len(image_out)-1):
